package com.aih21.team6.detection.tflite;
...
import com.aih21.team6.detection.MainActivity;
...
public class YoloV4Classifier implements Classifier {
    ...
    private ArrayList<Recognition> getDetectionsForFull(ByteBuffer byteBuffer, Bitmap bitmap) {
        ...
        System.out.println(OUTPUT_WIDTH_FULL[0]+" "+OUTPUT_WIDTH_FULL[1]+" "+labels.size());
...
//        }
    ...
    public boolean checkInvalidateBox(float x, float y, float width, float height, float oriW, float oriH, int intputSize) {
        ...
        float resize_ratio = Math.min(resize_ratioW, resize_ratioH); //min
        ...
        pred_coor[2] = Math.min(pred_coor[2], (oriW - 1));
        pred_coor[3] = Math.min(pred_coor[3], (oriH - 1));
